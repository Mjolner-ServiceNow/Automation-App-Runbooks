...
.DESCRIPTION
    This runbook disables an Active Directory user account by username.
    ...
    It requires Variables to be set for the domain, domain controller, and credentials. Remember to set these Variables in Azure Automation.

.PARAMETER Username
    The username of the Active Directory account to disable.
    This parameter accepts a string value and cannot be empty.

.EXAMPLE
    .\Disable-User.ps1 -Username "john"
...
try {
    ...
    $DisableADAccountParams = @{
        Identity   = $User
        Server     = $DomainController
        Credential = $Credentials
        PassThru   = $true
    }
    $User = Disable-ADAccount @DisableADAccountParams
    
    Write-Verbose "User account disabled successfully"
}
catch {
    ...
    if ($User) {
        ...
        $User | Select-Object -Property Enabled, SamAccountName, UserPrincipalName | ConvertTo-Json -WarningAction SilentlyContinue
