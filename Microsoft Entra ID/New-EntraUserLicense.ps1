<#
.SYNOPSIS
    Assigns a license to a Microsoft Entra ID user.

.DESCRIPTION
    This runbook assigns a Microsoft 365 license (SKU) to an existing Microsoft Entra ID user account.
    The user must have a usage location assigned before a license can be applied.
...
.PARAMETER UserPrincipalName
    The user principal name (UPN) of the user to assign the license to. Example: "user@contoso.com"

.PARAMETER SkuId
    The SKU ID (GUID) of the license to assign. Example: "6fd2c87f-b296-42f0-b197-1e91e994b900" (Office 365 E3)

.EXAMPLE
    .\New-EntraUserLicense.ps1 -UserPrincipalName "jane.doe@contoso.com" -SkuId "6fd2c87f-b296-42f0-b197-1e91e994b900"
...
try {
    ...
    # Get the user
    $User = Get-MgUser -Filter "userPrincipalName eq '$UserPrincipalName'" -Property Id, DisplayName, UserPrincipalName, UsageLocation
    if ($null -eq $User) {
        throw "Cannot find user. The user '$UserPrincipalName' does not exist."
    }

    # Verify the user has a usage location set
    if ([string]::IsNullOrEmpty($User.UsageLocation)) {
        throw "Missing usage location for user '$UserPrincipalName'. A valid usage location must be assigned to the user before assigning a license."
    }
    ...
    Write-Verbose "License '$($Sku.SkuPartNumber)' assigned successfully"
}
catch {
